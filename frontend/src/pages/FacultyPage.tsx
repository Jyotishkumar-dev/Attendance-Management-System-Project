...
const FACULTY: Faculty[] = [
  { id: "1", facultyId: "FAC-1001", name: "Dr. Anita Verma", department: "Computer Science", subject: "Data Structures", experience: "9 yrs", attendanceManaged: 94, status: "Active" },
  { id: "2", facultyId: "FAC-1002", name: "Prof. Rajesh Kumar", department: "Computer Science", subject: "Operating Systems", experience: "12 yrs", attendanceManaged: 89, status: "Active" },
  { id: "3", facultyId: "FAC-1003", name: "Dr. Meenal Joshi", department: "Electronics", subject: "Digital Circuits", experience: "7 yrs", attendanceManaged: 91, status: "On Leave" },
  { id: "4", facultyId: "FAC-1004", name: "Prof. Suresh Nair", department: "Mechanical", subject: "Thermodynamics", experience: "15 yrs", attendanceManaged: 86, status: "Active" },
  { id: "5", facultyId: "FAC-1005", name: "Dr. Kavita Rao", department: "Civil", subject: "Structural Engineering", experience: "10 yrs", attendanceManaged: 78, status: "Active" },
  { id: "6", facultyId: "FAC-1006", name: "Prof. Arvind Mehta", department: "Electronics", subject: "Signal Processing", experience: "6 yrs", attendanceManaged: 83, status: "Inactive" },
  { id: "7", facultyId: "FAC-1007", name: "Dr. Nisha Pillai", department: "Computer Science", subject: "Computer Networks", experience: "8 yrs", attendanceManaged: 96, status: "Active" },
  { id: "8", facultyId: "FAC-1008", name: "Prof. Sanjay Iyer", department: "Mechanical", subject: "Fluid Mechanics", experience: "11 yrs", attendanceManaged: 88, status: "On Leave" },
  { id: "9", facultyId: "FAC-1009", name: "Dr. Pooja Malhotra", department: "Civil", subject: "Surveying", experience: "5 yrs", attendanceManaged: 92, status: "Active" },
  { id: "10", facultyId: "FAC-1010", name: "Prof. Vikram Singh", department: "Computer Science", subject: "Algorithms", experience: "14 yrs", attendanceManaged: 90, status: "Active" },
];
...
function StatusBadge({ status }: { status: Status }) {
  const styles: Record<Status, string> = {
    ...
    "On Leave": "bg-amber-400/10 text-amber-300",
...
function AttendanceBar({ value }: { value: number }) {
  const color = value >= 85 ? "bg-emerald-400" : value >= 70 ? "bg-amber-400" : "bg-rose-400";
  return (
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <div className="text-sm font-semibold text-white/70">No faculty found</div>
      <p className="text-xs text-white/40 mt-1.5 max-w-xs">
        Try adjusting your search or filters, or add a new faculty member to get started.
      </p>
    </div>
  );
}
...
export function FacultyPage() {
  ...
  const avgManaged = Math.round(FACULTY.reduce((sum, f) => sum + f.attendanceManaged, 0) / FACULTY.length);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="font-display text-2xl font-semibold text-white">Faculty</h1>
            <span className="text-xs font-semibold text-white/40 border border-white/10 rounded-full px-2.5 py-1">
              {FACULTY.length} members
            </span>
          </div>
          <p className="text-sm text-white/40 mt-1">Manage faculty accounts and teaching assignments.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard label="Total Faculty" value={String(FACULTY.length)} icon={Users} tone="indigo" />
        <StatCard label="Active Faculty" value={String(active)} icon={UserCheck} tone="emerald" />
        <StatCard label="On Leave" value={String(onLeave)} icon={CalendarOff} tone="amber" />
        <StatCard label="Avg. Attendance Managed" value={`${avgManaged}%`} icon={TrendingUp} tone="emerald" />
      </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              placeholder="Search by name, faculty ID, or subject…"
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead className="sticky top-0 z-10 bg-[#12131a]/95 backdrop-blur-xl">
                <tr className="text-left text-[11px] uppercase tracking-wide text-white/30 border-b border-white/10">
                  ...
                  <th className="px-5 py-3.5 font-medium">Attendance Managed</th>
              ...
              <tbody>
                {paginated.map((f) => (
                  <tr key={f.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-colors group">
                    ...
                    <td className="px-5 py-3.5 font-mono text-xs text-white/50">{f.facultyId}</td>
                    <td className="px-5 py-3.5 text-white/60">{f.department}</td>
                    <td className="px-5 py-3.5 text-white/60">{f.subject}</td>
                    <td className="px-5 py-3.5 text-white/60">{f.experience}</td>
                    <td className="px-5 py-3.5">
                      <AttendanceBar value={f.attendanceManaged} />
                    </td>
                    <td className="px-5 py-3.5">
